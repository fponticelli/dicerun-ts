...
export class CMYK {
    static fromString(s) {
        const m = s.match(/^cmyk\((\d+(?:\.\d+)?)%, ?(\d+(?:\.\d+)?)%, ?(\d+(?:\.\d+)?)%, ?(\d+(?:\.\d+)?)%\)$/);
        if (m != null) {
            const [, cyan, magenta, y, k] = m;
            return new CMYK(parseFloat(cyan), parseFloat(magenta), parseFloat(y), parseFloat(k));
        }
        throw new Error(`Invalid CMYK string: ${s}`);
    }
    static ofChannels([cyan, magenta, yellow, black]) {
        return new CMYK(clamp(cyan * 100, 0, 100), clamp(magenta * 100, 0, 100), clamp(yellow * 100, 0, 100), clamp(black * 100, 0, 100));
    }
    cyan;
    magenta;
    yellow;
    ...
    equals(other, tollerance = TOLLERANCE) {
        return (nearEquals(this.cyan, other.cyan, tollerance) &&
